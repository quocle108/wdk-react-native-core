...
export function WdkAppProvider({
  ...
  useEffect(() => {
    ...
    const initializeWorklet = async () => {
      try {
        ...
        await WorkletLifecycleService.startWorklet(networkConfigs, bundleConfig)
        if (!cancelled) {
  ...
  }, [isWorkletInitialized, isWorkletLoading, isWorkletStarted, bundleConfig, networkConfigs])
